...
点群の間引き、外れ値除去、法線推定、剛体位置合わせを、既知の形状・変換・対応点・ラベルで定量評価する実験基盤です。

コマンドラインから実行できる7つの実験、入力が同じなら同じ結果を生成する合成データ、USGS 3DEP由来の公開サンプル、CSV形式の評価結果、比較図、再生成検証を含みます。

Python 3.10〜3.14でテストを自動実行し、Python 3.12では参照成果物を再生成して、CSVを許容誤差付きで照合し、要約の完全一致と比較図の形式・寸法を検査しています。評価条件、結果、主張できる範囲、制約の詳細は、以下の英語本文とリンク先の技術資料を参照してください。
...
Markdown, and PNG reference set. It compares CSV values with documented
numeric tolerances, requires an exact Markdown summary match, and checks the
PNG inventory, validity, and dimensions.
